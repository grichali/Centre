...
export class SeanceService {
  ...
  async modifySeance(seanceId: number, modifySeancenDto: ModifySeanceDto) {
    return await this.seanceRepository.modifySeance(seanceId, modifySeancenDto);
  }

  async deleteSeance(seanceId: number) {
    return await this.seanceRepository.deleteSeance(seanceId);
  }

  async integreFormation(seanceId: number, formationId: number) {
    return await this.seanceRepository.integreFormation(seanceId, formationId);
  }
}
